...
| 優先度 | 項目 | 說明 |
| --- | --- | --- |
| 高 | 真實照片不足 | 2026-06-24 Codex 已補第一輪首頁素材：Hero 南區門口、南區操作 / 說明情境、短影音縮圖；仍缺東區工作室真實照片。 |
| 中 | 手機第一屏未驗證 | CSS 有響應式，但仍需實機或截圖確認 5 秒內是否能看懂重點。 |
| 低 | Hero 右側缺圖 | 2026-06-24 Codex 已補南區門口真實照片；部署後需再做 live 截圖回讀。 |

## 後續處理方向

後續不要因為這份 review 又把首頁重做成新的 AI checklist。比較保守的下一步是：

1. 東區若要建立同等信任感，仍需補真實工作室照片；不要用南區或北區照片冒充東區。
2. 部署後重新做手機 / 桌機截圖，確認新增圖片沒有壓掉第一屏重點。
...
- 這輪沒有找到東區工作室真實照片；東區仍只用文字與 `/visit/?store=east` 導頁承接。
- 此段是 repo 修改狀態，正式站部署後仍需重新做 live HTML 與手機 / 桌機截圖回讀。
...
1. 未補東區工作室真實照片。
2. 未修 Cloudflare Worker CSP，僅完成 live 讀回與風險記錄。
