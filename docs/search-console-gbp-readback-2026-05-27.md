...
## 2026-05-27 21:00 授權後補做動作

使用者於同日後續明確授權可執行 live action 後，補做以下操作。

### Search Console

- 位置：同一個 URL Inspection 畫面。
- URL：`https://www.observe888.com/services/tainan-tuina/`。
- 操作：點擊 `要求建立索引`。
- 讀回確認：畫面顯示 `已要求建立索引`，並顯示 `已將網址加入優先檢索佇列。多次提交同一網頁並不會讓要求在佇列中的排位或優先順序有所改變。`
- 限制：這只代表已提交索引要求，不代表 Google 已完成收錄；仍需後續回查是否變成 `網址在 Google 服務中`。

### Google Business Profile

Google Business Profile 清單仍顯示：
...
- 北區店點入 `1 筆 Google 資訊更新` 後，後台不是顯示可逐項比較的 `接受` / `拒絕` 差異清單，而是開啟 `商家資訊` 檢查頁。
- 北區店檢查頁顯示 `檢查並修改商家資訊，有助提升你在 Google 的曝光率。請向下捲動並確認資訊無誤。`，展開後出現 `確認`。
- 北區店讀回的名稱、類別、描述、電話、網站、社群、服務清單連結、地址、服務範圍、營業時間與屬性沒有發現需要立即修正的明顯錯誤；已點擊 `確認`。
- 南區店點入 `1 筆 Google 資訊更新` 後，也開啟相同的 `商家資訊` 檢查頁。
- 南區店讀回的名稱、類別、描述、電話、網站、社群、服務清單連結、地址、服務範圍、營業時間與屬性沒有發現需要立即修正的明顯錯誤；已點擊 `確認`。

補做後讀回：

- 重新整理 `business.google.com/locations` 後，清單仍顯示 `Google 資訊更新 (2)`。
- 兩店列上仍各顯示 `1 筆 Google 資訊更新`。
- 因後台沒有顯示可逐項接受 / 拒絕的差異內容，且按 `確認` 後提示仍未消失，本輪只能記為「兩店商家資訊確認流程已按」，不能宣稱 `Google 資訊更新 (2)` 已清除。
- 本輪仍未修改 GBP 欄位、服務、產品、貼文、照片、地址、電話、網站、預約連結或營業時間。
- `缺少商家代碼 (1)` 未處理；原因是目前沒有既有專案檔案定義南區商家代碼，不應臨場自創代碼。
