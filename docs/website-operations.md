...
- 正式官網主專案：
  - 北區目前正式入口：`C:\見觀營運資料夾\01_官網_ObserveGeoPages`
...
- 廣告與事件監測：
  - 北區目前正式入口：`C:\見觀營運資料夾\02_廣告_AdsControl`
...
| --- | --- |
| 公開狀態 | 不再作為目前官網主入口 |
| 舊店點頁 | `https://www.observe888.com/north/` 應 `301` 到 `https://www.observe888.com/visit/` |
| 舊費用頁 | `https://www.observe888.com/north/pricing/` 應 `301` 到 `https://www.observe888.com/south/pricing/` |
| sitemap / 內鏈 | 不新增北區退場頁；只保留舊入口 fallback |
...
| 電話 | 0973-728-670 |
| 電話連結 | `tel:0973728670` |
| 官網店點頁 | `https://www.observe888.com/east/` |
| 導航助手 | `https://www.observe888.com/visit/?store=east` |
| 服務方式 | 純預約、二樓工作室；到場前可先用 LINE 或電話確認時段，電話與南區店相同 |
| 公開照片邊界 | 不公開可辨識外觀、門口、室內或能推回私人空間的照片；不把東區實拍列為官網或 Google 商家素材待辦 |
...
- `north/index.html`（只確認退場 fallback）
- `north/pricing/index.html`（只確認退場 fallback）
...
- 照片素材：南區優先使用真實門口、路口、室內、招牌、動線、服務空間；東區因住家兼工作室性質，不使用可辨識外觀、門口或室內實拍作為公開素材；避免過度修飾或醫療療效暗示。
- 網站 URL：南區與東區應連到各自官方店點頁或導航入口，不再連到 AdsControl 舊 public bundle；北區退場頁不再作為新內鏈、sitemap、Google Business 或外部名錄目標。
...
GEO / AEO 修改不得破壞真人閱讀。若一段內容只為了讓搜尋引擎或 AI 摘要抓取，優先放在 `title`、`description`、JSON-LD、FAQ schema、`llms.txt`、`sitemap.xml`、內鏈、footer 或低干擾區塊；不要把首頁主敘事、CTA 或服務主文改成機器問答、關鍵字清單或掃描器分數導向的段落。

可見主文只放真人也需要看到的資訊：地點、預約、費用、店別、服務方式、身體感受怎麼整理，以及急性或快速惡化時先就醫或諮詢。機器可讀內容可以更完整，但不能反過來主導頁面語氣。

優先補強的內容類型：

- FAQ：第一次來要看哪一頁、南區和東區怎麼選、是否需要先預約、如何看地圖、服務方式與費用在哪裡。
- 品牌說明：見觀結構如何看待身體感受、姿勢、承重、左右差、張力與生活負荷。
- 服務邊界：品牌主張不是診斷、治療、保證改善；急性或惡化症狀應先就醫。若為匿名患者或故事角色的主觀感受，可出現前後差異或個人回饋，但不可寫成官方保證。
- 店點資訊：南區店、東區工作室的名稱、地址、電話、網站、導航入口；北區只描述退場轉址，不再當目前店點入口。
- 搜尋問法：骨刺、手麻、腰椎退化、神經壓迫、坐骨神經痛等只能作為問法與觀察入口。

## 禁止文案

不要在官網、FAQ、Google 商家描述或照片說明中把下列內容寫成品牌主張、官方保證或普遍結果：
